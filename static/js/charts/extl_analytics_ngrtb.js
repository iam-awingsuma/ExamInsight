...
(() => {
  ...
  function setEmpty(elId, msg = "No data available.") {
    return setMessage(elId, `<p class="text-muted text-sm fst-italic p-2 mb-0">${msg}</p>`);
  }
  ...
  async function renderStanineThresholdPie({
    ...
    try {
      ...
      const trace = {
        ...
        marker: {
            colors: ['#FF5A5A', '#FFDEB9'] // Custom colors for better distinction
        },
        hovertemplate:
            "<b>%{label}</b><br>" +
            "Students: %{value}<br>" +
  ...
  function wireResize() {
    window.addEventListener("resize", () => {
      const el5 = document.getElementById("pie-st5-extl-ngrtb");
      const el6 = document.getElementById("pie-st6-extl-ngrtb");
